...
Якщо після вводу імені таблиці користувач вводить крапку `.`, то це означає, що він хоче посилатися на поле цієї таблиці. Наприклад, якщо користувач вводить `!users.`, то це означає, що він хоче посилатися на поле таблиці `users` і потрібно вивести список доступних полів цієї таблиці. Якщо після вводу імені функції користувач вводить круглі дужки `()`, то це означає, що він хоче викликати цю функцію. Наприклад, якщо користувач вводить `$sum()`, то це означає, що він хоче викликати функцію `sum`.

Для функцій якось потрібно виводити підказки по аргументам, які вони приймають. Наприклад, якщо користувач вводить `$sum(`, то потрібно вивести підказку, що ця функція приймає одне поле таблиці як аргумент.
...
_#f_ - це посилання на поле таблиці БД, яке може бути використано як аргумент для функції.
